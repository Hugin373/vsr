...
# ---------------------------------------------------------------------------------------------
# PRE-COMMITTED PROTOCOL — committed 2026-07-19 in a6a4f7f, before any sweep result existed.
...
# quantity -> (path, direction). "lower" means higher-is-better (lower BOUND).
GATED = {
    "A.r_ratio_gap": (("check_a", "r_ratio_gap"), "lower"),
    "A.retained_range": (("check_a", "retained_range"), "lower"),
    "A.weakest_stratum_r_gap": (("check_a", "weakest_stratum_r_gap"), "lower"),
    "A.weakest_stratum_range": (("check_a", "weakest_stratum_range"), "lower"),
    "B.eta2_pairing_ratio": (("check_b", "eta2_pairing_ratio"), "upper"),
    "B.eta2_near_cat_ratio": (("check_b", "eta2_near_cat_ratio"), "upper"),
    "B.eta2_far_cat_ratio": (("check_b", "eta2_far_cat_ratio"), "upper"),
    "B.eta2_near_cat_far_depth": (("check_b", "eta2_near_cat_far_depth"), "upper"),
    "B.eta2_far_cat_far_depth": (("check_b", "eta2_far_cat_far_depth"), "upper"),
    "B.eta2_pairing_clamp": (("check_b", "eta2_pairing_clamp"), "upper"),
    "B.max_abs_role_imbalance": (("check_b", "max_abs_role_imbalance"), "upper"),
    "C.clamped_fraction": (("check_c", "clamped_fraction"), "upper"),
    "C.max_stratum_clamped_fraction": (("check_c", "max_stratum_clamped_fraction"), "upper"),
}
...
def compute_bounds(per_seed: list[dict]) -> dict:
    ...
    out: dict[str, dict] = {}
    for name, (path, direction) in GATED.items():
...
def main() -> int:
    ...
    for s in per_seed:
        print(
            ...
            f"{s['check_a']['retained_range']:7.3f} {s['check_c']['clamped_fraction']:8.3f} "
    ...
    print(
        ...
        f"{reference['check_c']['clamped_fraction']:8.3f} "
