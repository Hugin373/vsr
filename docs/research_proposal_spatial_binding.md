...
1. **Probing grid over FIVE functional stages** (architectures are not homologous — projector vs resampler vs interleaving; binding is emergent over layers — so the sites are defined **functionally** and mapped per architecture to concrete tensors): **(1)** vision-encoder representation → **(2)** vision→language interface output → **(3)** early multimodal LM representation → **(4)** object-conditioned language representation → **(5)** answer/readout representation. Per layer, per quantity (ordinal/ratio/absolute; depth vs lateral axis).
   ...
   **Adopted standards (Cui et al. v2):** probe mask-pooled object tokens AND all-token/strip-level representations (spatial signal is distributed across background tokens; object-pooled-only probing underestimates what survives); two-ordering MCQ evaluation (⚠ Cui do the general two-ordering check; the strict both-pass rule + adjusted chance is OURS, DR3-r2 #9, not attributed to them); random-direction nulls; fixed-α and per-example-α (Probe*) reporting.
   ...
   **Three estimators, not one.** (a) mask-pooled (leaky; reported for comparison); (b) fixed-grid **strip** probes (leak-free — not selected by object position); (c) the **contrastive-pair estimator** — pairs **matched on mask geometry, differing in true depth**, decoding depth from the *difference* of their activations. ⚠ **Corrected 2026-07-17 (ruling): they need NOT agree in MAGNITUDE.** (c) conditions away part of B1 (monocular cues) by matching mask geometry, so it answers a **narrower** question than (a)/(b); "agreement" means a **consistent QUALITATIVE story** (same sign, same site/layer pattern), never matched numbers. Where the ceiling *subtracts* the leak, (c) makes it **inexpressible by construction**: identical mask geometry ⇒ no function of mask geometry can separate the pair ⇒ the dumb-features probe is at chance **by design**. ⚠ Its matched pairs must be **rendered on purpose** (a stimulus-battery deliverable) — post-hoc matching within a tolerance leaves exactly the residual the probe would exploit.

   **Prior art, cited explicitly:** the dumb-features ceiling is a **descendant of control tasks** (Hewitt & Liang 2019; Belinkov 2022), and the contrastive-pair estimator descends from **Why Far Looks Up's minimal contrastive pairs** — neither is a new invention. **Our contribution is narrower and more defensible: the SELECTION leak** — *the pooled vector is chosen BY the object's image position, so selection and stimulus geometry already contain most of the answer* — together with the incremental-gain form of the ceiling and the application of matched pairs to that leak. ⚠ **Wording corrected 2026-07-17 (DR3-r7 / annex A1):** this said *"the selection IS the answer"*. Our R² = 0.942 proves **stimulus confounding** and invalidates raw scores; it does **not** isolate the **selection mechanism** from **positional representation**. The strong form is licensed only after A1's isolation battery. Until then: **candidate** contribution.
